...
## 0. À LIRE EN PREMIER — état d'avancement & suivi
...
* **AUCUNE CARTE SANS SOURCE VALIDE** : Une carte de loi ne doit jamais apparaître si son `sourceUrl` ou `textUrl` est en 404, **ou si la page ne correspond pas au texte annoncé**. Chaque URL de `src/lib/laws.js` porte un fragment de titre officiel attendu (`sourceExpect`/`textExpect`) vérifié par `npm run check:sources` (CI bloquante à chaque push + cron quotidien `check-sources.yml` qui ouvre une issue en cas de lien mort). Un statut HTTP 200 seul ne suffit pas : l'audit du 2026-07-14 a trouvé des liens 200 pointant vers de mauvais dossiers.
* **AUCUN STATUT INVENTÉ** : `upcoming` exige un dossier législatif en cours vérifié ; `passed` exige la page officielle du scrutin avec la répartition des votes extraite de cette page. En l'absence de scrutin vérifié à venir, la section affiche un état vide honnête. Audit et plan complet : `docs/plans/lois-sources-fiabilite.md`.
