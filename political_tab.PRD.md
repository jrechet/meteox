...
- [x] Job serveur `sync-dossiers` : alimentation « Prochains scrutins » (17e législature) — staging `dossier_candidates` + admin de validation humaine (PR #29-#32), enrichi initiateur/cosignataires (PR #38) ; déployé en int
- [x] **Élargissement du corpus des lois votées** (issue #3, tâche 3) : job `sync-corpus` — détection dans le jeu Scrutins AN (lég. 16+17) des votes sur l'ensemble d'un texte de loi (solennels et ordinaires) à thème environnemental, adoptés depuis 2023 → staging `law_candidates` + section admin « Lois votées candidates » (promotion humaine avec titre éditorial, dossier officiel vérifié et votes par bloc ; 29 candidats détectés sur les données réelles 2023-2026)
